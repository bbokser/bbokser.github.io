...
comments: true
date: "2023-04-17"
title: Beam Optimization and Material Choice for Robot Components
math: katex
---
...
Finally, I found that deciding on minimum thicknesses $t_{min}$ is very tricky. In the end, I went with the pragmatic approach of checking the minimum tube thicknesses available online for each material. For titanium and aluminum, that's 0.4064 mm (~1/64"), whereas for CFRP it's 0.762 mm (~0.03"). However, I think that a deeper analysis would need to be performed in a more realistic situation where one might be concerned about side impacts.

## Results
I used [NLOPT](https://nlopt.readthedocs.io/en/latest/NLopt_Python_Reference/), a library for nonlinear optimization, to solve the aforementioned optimization problem. My code can be found [here](https://github.com/bbokser/beam-opt).

| Material      | R (mm) | r (mm) | Thickness (mm) | Mass (kg) |
| -----------   | -----------          | -----------    | -----------    | -----------      |
| [Annealed Grade 5 Titanium](https://www.makeitfrom.com/material-properties/Annealed-Grade-5-Titanium) | 33.978    | 33.580          | 0.395          | 0.112            |
| [Aluminum 7075-T6](https://www.makeitfrom.com/material-properties/7075-T6-Aluminum)          | 39.64     | 39.252          | 0.394          | 0.0795             |
| [CFRP Tube, 0/90 Ply](http://www.performance-composites.com/carbonfibre/mechanicalproperties_2.asp)       | 20.834    | 20.072           | 0.762          | 0.0286             |


## Analysis
After looking at the equations from earlier, one's first impression might be that the cylinder radius should jump to the maximum radius possible. This is not the case, because doing so would enlarge the cross-sectional area (and therefore mass) unnecessarily. Instead, it's all about the minimum wall thickness parameter.
...
Note that if minimum wall thickness were no object (i.e. you could have infinitesimal walls), then the optimal choice *would* be to maximize your radius! Sadly, we live in the real world, where we have to care about manufacturability/fabricatability/etc.
...
## Dethroning CFRP
Despite the doubled safety factor and higher minimum thickness, CFRP is the clear winner in our example scenario. But is it always? Let's see if we can dethrone CFRP by hitting it where it hurts: the shear strength. As you may have noticed from the first chart, CFRP has a much lower shear strength than either aluminum or titanium. We can increase shear stress by increasing the axial offset distance $L_y$:

![Axial Offset Comparison](/images/2023-04-17/compare-offset.png)

You can see that as $L_y$ (and therefore torsion) increases, the CFRP begins to lose its advantage over the other two options. The other two remain constant because their shear stress limit has not yet been reached.

## How to Actually Choose Your Material
Here's another thing you may be wondering. 

    "Why is the optimal titanium beam heaviest if titanium has a higher strength-to-weight ratio than aluminum? Shouldn't titanium always be the better choice of the two from a theoretical standpoint?"

I have to admit I was initially left scratching my head after looking at these plots. But the reason for this is that I've set up the optimization problem to find the geometry that provides the lowest mass per material. 

Recall Equations 5 through 8: the torsional stress, tensile stress, and deflection are all inversely proportional to the 4th power of the radius, whereas area (and therefore mass) is proportional to the 2nd power of the radius. This means that geometry can overcome inferior material properties when it comes to saving weight!

    "Okay, so we should 3D print everything out of foaming PLA instead of using metal?"

Absolutely not! For example, if we were to reduce the outer radius of the aluminum beam down to that of the optimal titanium beam, it would need to be much thicker to meet the requirements. Let's see how that looks: We'll make the outer radius $R$ constant across all materials.

![Axial Offset Comparison](/images/2023-04-17/compare-offset.png)

Notice how the optimal beam radii of the titanium, CFRP, and aluminum beams are inversely proportional to their respective stiffnesses and strength-to-weight ratios: aluminum, having the lowest stiffness, needs the largest radius to hit the deflection requirement.
...
Of course, CFRP has both the lowest density and the highest stiffness, which is why it performs so well when we have free reign over the geometry.
    ...
    "So when is titanium actually better than aluminum?"
...
- Side impacts are an important consideration which I completely ignored in this analysis (and it's one of the reasons I don't think CFRP bikes are worth the cost).
...
- Thermal properties: titanium and anodized aluminum win here.

...
Note the $A$ matrix (AKA "the state matrix"), which governs how $X$ affects $\dot{X}$. In this case, the only relation is that $\dot{z} = \dot{z}$, which is pretty self-explanatory. That leads to the matrix shown.
...
## Next Steps
In the next part of this series, I will cover how contact is simulated, so that the point mass can bounce off the ground, and how to extend the simulation from 1D to 2D.